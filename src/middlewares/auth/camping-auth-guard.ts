...
import { getUniqueCampingEmployee } from "@/db/actions/camping/employee.actions";
import { prisma } from "@/db";
import { NODE_ENV } from "@/configs/env.ts";
import { logger } from "@/utils/logger";

export const campingAuthGuard = (type?: CampingEmployeeRoleType[], customErrorMessage?: string) =>
	createMiddleware<{
		Variables: {
			user_id: string;
			client_id: string;
		...
		if (JWT.isImpersonationToken(authToken)) {
			...
			const user = JWT.verifyCampingAuthToken(authToken);
			userId = user.id;
		}

		const employee = await getUniqueCampingEmployee({
			id: userId,
		});
		...
		const client_id =
			employee.type === "admin"
				...
				: (employee.employee as any).client_id;
		...
		context.set("user", employee.employee as client);
