...
import { createEmployeeRequestBodyValidator } from "delivery/validators/employee.validators.ts";
...
export const createEmployeeHandler = createHandlers(
	...
	async (context) => {
		...
		// Normalize name — works for both union branches
		...
		const response = {
			...
			...resolveMessageTemplate("delivery.employee.create.SUCCESS", { id: employee.id }),
			data: {
		...
		return context.json<APIResponse<{ vertical_delivery_employee: ReturnType<typeof withFullName<typeof employee>> }>>(
			response,
