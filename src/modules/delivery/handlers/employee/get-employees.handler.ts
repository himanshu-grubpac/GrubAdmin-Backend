...
import { getEmployeesRequestQueryValidator } from "delivery/validators/employee.validators.ts";
import {
	getDeletedVerticalDeliveryEmployees,
	getVerticalDeliveryEmployees,
...
export const getEmployeesHandler = createHandlers(
	...
	async (context) => {
		...
		if (with_permission_for_box_id) {
			const blockedPerms = await prisma.vertical_delivery_employee_box.findMany({
				where: {
		...
		const employeesData =
			status === "deleted"
				? await getDeletedVerticalDeliveryEmployees({
					client_id,
				...
				: await getVerticalDeliveryEmployees({
		...
		if (with_permission_for_box_id) {
			const perms = await prisma.vertical_delivery_employee_box.findMany({
				where: {
					box_id: with_permission_for_box_id,
