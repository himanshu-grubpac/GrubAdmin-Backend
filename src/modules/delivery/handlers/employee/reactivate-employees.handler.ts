...
import { reactivateEmployeesRequestBodyValidator } from "delivery/validators/employee.validators.ts";
import { reactivateVerticalDeliveryEmployees } from "@/db/actions/vertical-delivery-employee.actions";
...
export const reactivateEmployeesHandler = createHandlers(
	...
	async (context) => {
		...
		const result = await reactivateVerticalDeliveryEmployees({
			ids,
			client_id,
		...
		if (skipped > 0) {
			...
			if (updated === 0) {
				throw new APIError(
					message,
					"delivery.restaurant.assign.manager.ACTIVATION_CONFLICT",
