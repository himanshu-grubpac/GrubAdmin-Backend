...
import { updateAccountRequestBodyValidator } from "delivery/validators/account.validators.ts";
	...
	async (context) => {
		...
		if (new_password || old_password) {
			...
			if (hasExistingPassword) {
				if (!old_password) {
					throw new APIError(undefined, "delivery.account.PASSWORD_REQUIRED", undefined, 400);
				}
				...
				if (!isOldPasswordCorrect) {
					throw new APIError(undefined, "delivery.auth.login.PASSWORD_INVALID", undefined, 400);
				}


				if (new_password && new_password === old_password) {
					throw new APIError(undefined, "delivery.account.SAME_PASSWORD", undefined, 400);
				}
			} else {
		...
		const isNameChanged =
			type === "admin"
				...
				: !!(
					full_name !== undefined &&
					(firstName !== (user as vertical_delivery_employee).first_name ||
						lastName !== (user as vertical_delivery_employee).last_name)
				);
		...
		if (isPhoneChanged && newPhone) {
			...
			if (existingPhone) {
				throw new APIError("This phone number is already in use by another account.", "delivery.account.PHONE_EXISTS", undefined, 409);
			}
		}
		...
		if (!has_changed) {
			const response = {
				...
				...resolveMessageTemplate("delivery.employee.profile.UPDATE_SUCCESS"),
