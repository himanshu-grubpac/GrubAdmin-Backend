...
import { confirmUpdateAccountRequestBodyValidator } from "delivery/validators/account.validators.ts";
import {
...
export const confirmUpdateAccountHandler = createHandlers(
	...
	async (context) => {
		...
		if (!updatedDetails) {
			...
			throw new APIError(undefined, "delivery.account.NO_CHANGE_REQUESTS", undefined, 400);
		}
		...
		const response = {
			...
			...resolveMessageTemplate("delivery.employee.profile.UPDATE_SUCCESS", { id: user.id }),
			is_otp: false,
			has_changed: true,
