...
import { editRestaurantRequestBodyValidator } from "delivery/validators/restaurant.validators";
...
export const editRestaurantHandler = createHandlers(
	...
	async (context) => {
		...
		if (type === "manager") {
			...
			if (managerObj.restaurant_id !== id) {
				throw new APIError(
					...
					"delivery.restaurant.update.ACCESS_DENIED",
					undefined,
					403
				);
			}
		...
		const response = {
			...
			...resolveMessageTemplate("delivery.restaurant.update.SUCCESS"),
