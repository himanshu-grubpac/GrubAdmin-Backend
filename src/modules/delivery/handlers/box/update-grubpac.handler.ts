...
import { updateGrubpacRequestBodyValidator } from "delivery/validators/box.validators.ts";
import { updateVerticalDeliveryGrubpac } from "@/db/actions/box.actions.ts";
...
export const updateGrubpacHandler = createHandlers(
	...
	async (context) => {
		...
		const box = await updateVerticalDeliveryGrubpac({
			id,
			name,
		...
		const response = {
			...
			...resolveMessageTemplate("delivery.box.settings.SUCCESS", { id: box.id }),
