...
import { setNewPasswordRequestBodyValidator } from "delivery/validators/auth.validators.ts";
...
export const setNewPasswordHandler = createHandlers(
    ...
    async (context) => {
        ...
        if (authHeader && authHeader.startsWith("Bearer ")) {
            ...
            if (!token) {
                throw new APIError(undefined, "delivery.auth.login.AUTH_TOKEN_REQUIRED");
            }
            ...
            try {
                ...
                if (decoded.type !== "password_reset") {
                    throw new APIError(undefined, "delivery.auth.login.INVALID_AUTH_TOKEN");
                }
                ...
                if (email) {
                    ...
                    if (email && employeeByToken?.employee.email !== email) {
                        throw new APIError(undefined, "delivery.auth.login.CREDENTIAL_MISMATCH");
                    }
                }
            } catch (error) {
                ...
                if (!savedOtp || !(await compareOtp(bodyToken, savedOtp.otp))) {
                    throw new APIError(undefined, "delivery.auth.login.INVALID_OTP_TOKEN");
                }
        ...
        if (!userId) {
            throw new APIError(undefined, "delivery.auth.login.AUTH_FAILED");
        }
