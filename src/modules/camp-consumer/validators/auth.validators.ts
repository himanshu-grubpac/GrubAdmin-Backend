...
export const sendOtpRequestBodyValidator = zValidator(
	...
	z
		.object({
			...identityFields,
			for_what: z.enum(["login", "register"]).optional(),
...
export const verifyOtpRequestBodyValidator = zValidator(
	...
	z
		.object({
			...identityFields,
			otp: z
				.string({ error: "Please provide an otp" })
				.trim()
	...
	z
		.object({
			...
			password: z
				...
				.max(20, { error: "Password can be at max 20 characters long" }),
		})
		.refine(identityRefine, identityRefineOptions)
		.transform(identityTransform),
	(response) => {
...
export const setNewPasswordRequestBodyValidator = zValidator(
	...
	z
		.object({
			...
			auth_token: z.string({ error: "Please provide an auth token" }).optional(),
			"auth-token": z.string({ error: "Please provide an auth token" }).optional(),
			token: z.string({ error: "Please provide an auth token" }).optional(),
			password: z
				.string({ error: "Please provide a password" })
				.trim()
				.min(8, { error: "Password must be at least 8 characters long" })
				.max(20, { error: "Password can be at max 20 characters long" }),
			confirm_password: z
				.string({ error: "Please provide a confirm password" })
				.trim()
				.min(8, { error: "Password must be at least 8 characters long" })
				.max(20, { error: "Password can be at max 20 characters long" }),
		})
