...
import { updateAccountRequestBodyValidator } from "food/validators/account.validators.ts";
	...
	async (context) => {
		...
		if (new_password || old_password) {
			...
			if (hasExistingPassword) {
				if (!old_password) {
					throw new APIError(undefined, "food.account.PASSWORD_REQUIRED", undefined, 400);
				}
				...
				if (!isOldPasswordCorrect) {
					throw new APIError(undefined, "food.auth.login.PASSWORD_INVALID", undefined, 400);
				}


				if (new_password && new_password === old_password) {
					throw new APIError(undefined, "food.account.SAME_PASSWORD", undefined, 400);
				}
			} else {
		...
		const isNameChanged =
			type === "admin"
				...
				: !!(
					full_name !== undefined &&
					(firstName !== (user as vertical_food_employee).first_name ||
						lastName !== (user as vertical_food_employee).last_name)
				);
		...
		if (!has_changed) {
			const response = {
				...
				...resolveMessageTemplate("food.employee.profile.UPDATE_SUCCESS"),
