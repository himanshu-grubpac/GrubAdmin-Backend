...
import { createEmployeeRequestBodyValidator } from "food/validators/employee.validators.ts";
...
export const createEmployeeHandler = createHandlers(
	...
	async (context) => {
		...
		const response = {
			...
			...resolveMessageTemplate("food.employee.create.SUCCESS", { id: employee.id }),
			data: {
		...
		return context.json<APIResponse<{ vertical_food_employee: ReturnType<typeof withFullName<typeof employee>> }>>(
			response,
