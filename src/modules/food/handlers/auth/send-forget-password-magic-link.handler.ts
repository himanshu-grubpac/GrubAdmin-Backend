...
import { sendForgetPasswordMagicLinkRequestBodyValidator } from "food/validators/auth.validators.ts";
...
export const sendForgetPasswordMagicLinkHandler = createHandlers(
	...
	async (context) => {
		...
		if (!updatedOtpRecord) {
			throw new APIError(undefined, "food.auth.login.MAGIC_LINK_SAVE_FAILED");
		}
