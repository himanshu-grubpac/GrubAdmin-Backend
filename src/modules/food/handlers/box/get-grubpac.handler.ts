...
import { getBoxesRequestQueryValidator } from "food/validators/box.validators.ts";
        ...
        if (group_by === "restaurants" || group_by === "restaurant") {
			...
			const response = {
				...
				data: {
					...
					count: totalCountAcrossGroups, // Sum of all items in matching groups
					total_count: count // Total items matching the DB query (ignore handler filter for this)
				},
				pagination: calculatePagination(finalPage, finalLimit ?? count, count),
			};
