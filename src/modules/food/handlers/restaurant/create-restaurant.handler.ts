...
import { createRestaurantRequestBodyValidator } from "food/validators/restaurant.validators";
	...
	async (context) => {
		...
		await loggerService.log({
			...
			actor: {
				...
				name: actorName,
				role: userType,
				table: userType === "admin" ? "client" : "vertical_food_employee",
			},
			client_id,
			subject: {
				id: restaurant.id,
		...
		const response = {
			...
			...resolveMessageTemplate("food.restaurant.create.SUCCESS", { id: restaurant.id, name: restaurant.name }),
			data: {
