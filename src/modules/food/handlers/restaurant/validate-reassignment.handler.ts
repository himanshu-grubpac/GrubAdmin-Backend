	...
	async (context) => {
		...
		if (restaurant_ids.includes(destination_restaurant_id)) {
			throw new APIError(
				...
				"food.restaurant.reassign.SELF_REASSIGNMENT",
				undefined,
				400
			);
		}
		...
		if (sourceRestaurants.length !== restaurant_ids.length) {
			throw new APIError(
				...
				"food.restaurant.reassign.SOURCE_NOT_FOUND",
				undefined,
				404
			);
		}
		...
		if (!destRestaurant) {
			throw new APIError(
				...
				"food.restaurant.reassign.DESTINATION_NOT_FOUND",
				undefined,
				404
			);
		}
		...
		if (destRestaurant.status !== "active") {
			throw new APIError(
				...
				"food.restaurant.reassign.DESTINATION_NOT_ACTIVE",
				undefined,
				409
			);
		}
		...
		const movingManagers = await prisma.vertical_food_employee.findMany({
			where: {
