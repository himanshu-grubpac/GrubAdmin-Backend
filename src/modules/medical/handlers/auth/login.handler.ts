...
import { loginRequestBodyValidator } from "medical/validators/auth.validators";
import { activateMedicalEmployee, getUniqueMedicalEmployee } from "@/db/actions/medical/employee.actions";
...
export const loginHandler = createHandlers(
	...
	async (context) => {
		...
		if (employee.employee.status === "suspended") {
			throw new APIError("Your account has been suspended!", "medical.auth.login.SUSPENDED", { is_account_found });
		}
		...
		if (!employee.employee.password) {
			throw new APIError(
				...
				"medical.auth.login.PASSWORD_NOT_SET",
				{ is_account_found, is_password_set }
			);
		}
		...
		if (!isCorrectPassword) {
			throw new APIError(
				...
				"medical.auth.login.INVALID_CREDENTIALS",
				{ is_account_found, is_password_set }
			);
		}
		...
		await loggerService.log({
			...
			actor: {
				...
				table: employee.type === "admin" ? "client" : "vertical_medical_employee",
			},
			client_id,
			subject: {
				id: emp.id,
		...
		return context.json<APIResponse<ResponseData>>({
			success: true,
			client_id,
			...resolveMessageTemplate("medical.auth.login.SUCCESS"),
			data: {
				auth_token: token,
				is_account_found,
				is_password_set,
