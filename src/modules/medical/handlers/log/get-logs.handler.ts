...
import {
	...
	categoryToTypes,
	filterStructure,
} from "medical/validators/log.validators.ts";
...
export const getSystemLogsHandler = createHandlers(
	...
	async (context) => {
		...
		const response = {
			...
			...resolveMessageTemplate("medical.common.FETCH_SUCCESS"),
		...
		return context.json<APIResponse<typeof response.data>>(response, response.code as 200);
	},
);
