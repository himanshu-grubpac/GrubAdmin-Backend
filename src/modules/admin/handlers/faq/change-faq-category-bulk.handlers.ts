...
import { changeBulkFaqCategoryRequestBodyValidator } from "@/modules/admin/validators/faq.validators.ts";
import { authGuard } from "@/middlewares/auth";
import {
	changeFaqQuestionsCategory,
	getFaqQuestions,
...
export const changeFaqCategoryBulkHandler = createHandlers(
	authGuard(["employee", "admin"]),
	changeBulkFaqCategoryRequestBodyValidator,
	ipMiddleware,
	async (context) => {
		...
		const { ids, new_category, old_category } = context.req.valid("json");

		const updatedFaqs = await getFaqQuestions({
			ids,
		});

		await changeFaqQuestionsCategory({
			question_ids: ids,
