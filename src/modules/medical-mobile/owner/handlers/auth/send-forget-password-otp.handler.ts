...
import { sendForgetPasswordOtpRequestBodyValidator } from "@/modules/medical-mobile/owner/validators/auth.validators.ts";
...
export const sendForgetPasswordOtpHandler = createHandlers(
	...
	async (context) => {
		...
		const is_account_found = !!employee && employee.type === "admin";
		...
		const { email: sentToEmail } = await sendOtpToOwner(employee, "forget_password");
