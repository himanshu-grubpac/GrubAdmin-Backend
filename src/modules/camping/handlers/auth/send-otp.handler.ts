...
import { sendOtpRequestBodyValidator } from "camping/validators/auth.validators.ts";
...
export const sendOtpHandler = createHandlers(
	...
	async (context) => {
		...
		if (clientRecord.status !== "active" && clientRecord.status !== "inactive") {
			throw new APIError(undefined, "camping.auth.login.ACCOUNT_INACTIVE");
		}
		...
		let savedOtp = null;
		if (target_otp_id) {
			savedOtp = await getSavedCampingOtp(clientEmail, target_otp_id);
		} else {
			savedOtp = await getSavedCampingOtp(clientEmail);
		}
		...
		await services.mailer.sendEmail({
			...
			subject: "Camping Portal - Login OTP",
			to: clientEmail,
			text: `Your OTP to log into your camping platform is ${otp} (OTP Session ID: ${otp_id})\n\nfor_what: login-send`,
		});
