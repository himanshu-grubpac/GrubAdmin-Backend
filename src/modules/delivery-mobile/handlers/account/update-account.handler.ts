...
import { updateAccountRequestBodyValidator } from "delivery-mobile/validators/account.validators.ts";
...
export const updateAccountHandler = createHandlers(
    ...
    async (context) => {
        ...
        if (email) {
            ...
            const employeeProfileUpdateOtp = await getDeliveryEmployeeUpdateOtp(
                user.id,
            );
