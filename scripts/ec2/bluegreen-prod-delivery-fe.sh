#!/usr/bin/env bash
# Production Delivery FE blue-green (pilot EC2).
# Green = :3010 (grub-delivery-frontend / compose grubdelivery-fe-prod-green)
# Blue  = :3013 (grub-delivery-frontend-blue / compose grubdelivery-fe-prod-blue)
# nginx fe_delivery_prod via /etc/nginx/snippets/grubdelivery-fe-prod-upstream.conf
set -euo pipefail

STATE_DIR="/var/lib/grubadmin"
STATE_FILE="${STATE_DIR}/active-prod-delivery-fe.color"
PREVIOUS_STATE_FILE="${STATE_DIR}/previous-prod-delivery-fe.color"
NGINX_SNIPPET="/etc/nginx/snippets/grubdelivery-fe-prod-upstream.conf"
NGINX_SITE="/etc/nginx/sites-available/grubdelivery-fe"
DEPLOY_PATH="/home/ubuntu/grubdelivery-frontend"
...
GREEN_PORT="3010"
BLUE_PORT="3013"
GREEN_CONTAINER="grub-delivery-frontend"
BLUE_CONTAINER="grub-delivery-frontend-blue"
GREEN_PROJECT="grubdelivery-fe-prod-green"
BLUE_PROJECT="grubdelivery-fe-prod-blue"
LEGACY_PROJECT="grubdelivery-fe-prod"

usage() {
  cat <<'EOF'
Usage: bluegreen-prod-delivery-fe.sh <command> [args]

Commands:
  init                 One-time: nginx snippet + state; map existing :3010 to green
  ...
  wait-http <port>     Block until HTTP response on 127.0.0.1:<port>/ (allows 3xx)
  cutover <color>      Switch nginx fe_delivery_prod to color
  rollback             Switch nginx back to previous color if healthy
  start-warm-standby   Start inactive color container; nginx unchanged
EOF
}
...
wait_http() {
  ...
  for ((i = 1; i <= max_attempts; i++)); do
    # Delivery FE often returns 307 to login — treat 2xx/3xx as healthy
    code="$(curl -s -o /dev/null -w '%{http_code}' "http://127.0.0.1:${port}/" || true)"
    if [[ "$code" =~ ^(2|3)[0-9][0-9]$ ]]; then
      echo "HTTP ${code} OK on Delivery FE port ${port} (attempt ${i})"
      return 0
    fi
    echo "waiting for Delivery FE HTTP on port ${port} (attempt ${i}/${max_attempts}, last=${code})..."
    sleep 3
  done
  echo "Delivery FE HTTP failed on port ${port} after ${max_attempts} attempts" >&2
  return 1
}

ensure_snippet_included() {
  if ! grep -q 'grubdelivery-fe-prod-upstream.conf' "$NGINX_SITE" 2>/dev/null; then
    echo "WARN: ${NGINX_SITE} must include ${NGINX_SNIPPET} (replace inline upstream fe_delivery_prod)" >&2
  fi
}
...
cmd_init() {
  ...
  echo "Delivery FE blue-green init complete. Active color: $(read_active_color)"
...
cmd_status() {
  ...
  echo "--- docker ---"
  docker ps --format "table {{.Names}}\t{{.Ports}}\t{{.Status}}" | grep -E 'delivery-frontend|NAMES' || true
}
...
cmd_cutover() {
  ...
  echo "Cutover complete: fe_delivery_prod -> ${target} (127.0.0.1:${port})"
}
...
cmd_rollback() {
  ...
  echo "Rollback complete: fe_delivery_prod -> ${prev} (127.0.0.1:${port})"
}
...
cmd_start_warm_standby() {
  ...
  echo "Starting Delivery FE warm standby ${inactive} on :${port} from image ${image}"
